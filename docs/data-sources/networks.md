...
- `cidr` (String) Filter: exact cidr match (server-side). Applies when hub_id or scope_id is set.
- `hub_id` (Number) List all networks across all scopes of a hub.
- `id` (Number) Lookup a single network by ID.
- `name` (String) Filter: exact name match (server-side). Applies when hub_id or scope_id is set.
